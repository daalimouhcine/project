...
export default function ManagerDemandeAnnulationComponent({
  changedDemande,
  demandeInfo,
  reservedInfo,
}: any) {
  ...
  const [reserve, setReserve] = useState<any>(reservedInfo);
  const [demande, setDemande] = useState<any>(demandeInfo);
  const [collabDemandeInfo, setCollabDemandeInfo] = useState<any>({});

  useEffect(() => {
    setDemande(demandeInfo);
    setReserve(reservedInfo);
  }, [demandeInfo, reservedInfo]);
  ...
  const annulerReservation = () => {
    axios
      .delete("http://127.0.0.1:8000/api/reservations/" + reserve.id)
      .then((res) => {
        axios
          ...
          .then((res) => {
            ...
            Swal.fire({
              position: "center",
              icon: "success",
              title: "Vous avez accepter la demande d'annulation avec success",
  ...
  const refuserAnnulationReservation = () => {
    axios
      .delete("http://127.0.0.1:8000/api/demandes/" + demande.id)
      .then((res) => {
        setOpen(false);
        ...
        Swal.fire({
          position: "center",
          icon: "success",
          title: "Vous avez Refuser la demande d'annulation",
          showConfirmButton: false,
          timer: 2000,
        });
      });
              ...
              <div className='inline-block align-bottom bg-gray-100 text-center rounded-lg px-4 pt-5 pb-4 overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-sm sm:w-full sm:p-6'>
                <div className='flex-1 flex flex-col p-8'>
                  ...
                  <dl className='flex-grow flex flex-col justify-between'>
                    ...
                    <div
                      className='demandeMessage w-full h-full mt-3 rounded-md px-5 py-2 text-left bg-white'
                ...
                <div>
                  <div className='-mt-px flex divide-x divide-gray-200'>
                    ...
                    <div className='-ml-px w-0 flex-1 flex'>
                      <button
                        onClick={() => refuserAnnulationReservation()}
