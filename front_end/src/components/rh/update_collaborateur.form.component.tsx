...
const ModifierCollaborateurForm = ({ collaborateur, chengColl }: any) => {
  const [selectedColla] = React.useState({
    nom: collaborateur.nom,
    prenom: collaborateur.prenom,
    phone: collaborateur.phone,
    adresse: collaborateur.adresse,
    equepe: collaborateur.equepe,
  });
  ...
  } = useForm<dataCollaborateur>({ defaultValues: selectedColla });
  ...
  const editCollaborateur = (data: any) => {
    const dataCollborateur = {
      nom: data.nom,
      prenom: data.prenom,
      phone: data.phone,
    ...
    axios
      .put(
        `http://127.0.0.1:8000/api/collaborateurs/` + collaborateur.id,
        dataCollborateur
      )
      .then((res) => {
        if (res.data.status === 200) {
          ...
          Swal.fire({
            position: "center",
            icon: "success",
            title: "Collaborateur a été modifier avec success",
    ...
    <div
      ...
      <div className='modal_form'>
        <form
          ...
          onSubmit={handleSubmit(editCollaborateur)}>
...
export default ModifierCollaborateurForm;
